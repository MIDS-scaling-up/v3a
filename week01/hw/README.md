# HW01: Docker, Cloud ML Services (Sagemaker) and Pricing

*Complete Lab01 before starting HW01*
...
We will be using Nvidia A10 GPUs later in the class which are delivered through g5 AWS instances. Please raise a limit request for "Running On-Demand G and VT instances" at 64 VCPUs which will enable these instances in the region of your choice (eg. US-East-1). Please request on-demand instance type (as opposed to spot). The `Service Quotas` option can be found via the top search box.   
...
Sample message : 
```
Please update the limit for VCPU on my account to be 64 VCPUs for the g5 instance type. We are expecting a graded homework in our class at UC Berkeley for the Masters in Data Science program on training large Deep Learning models.
```
